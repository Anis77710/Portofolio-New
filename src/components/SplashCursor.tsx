...
// Theme-specific color configurations for the splash effect
const getThemeColors = (theme: BackgroundTheme) => {
  const colors: Record<BackgroundTheme, { r: number; g: number; b: number }> = {
    midnight: { r: 0.0, g: 0.8, b: 0.7 },    // Cyan
    aurora: { r: 0.2, g: 0.9, b: 0.5 },      // Green
    sunset: { r: 1.0, g: 0.5, b: 0.2 },      // Orange
    ocean: { r: 0.6, g: 0.3, b: 1.0 },       // Purple
  };
  return colors[theme];
};
...
export const SplashCursor = () => {
  ...
  const getSupportedFormat = (gl: WebGL2RenderingContext, internalFormat: any, format: any, type: any) => {
  ...
  const supportRenderTextureFormat = (gl: WebGL2RenderingContext, internalFormat: any, format: any, type: any) => {
  ...
  useEffect(() => {
    ...
    const pointers: any[] = [];
    const Pointer = pointerPrototype as any;
    ...
    function onTouchMove(e: TouchEvent) {
      ...
      for (let i = 0; i < touches.length; i++) {
        ...
        const offsetX = touches[i].clientX - rect.left;
        const offsetY = touches[i].clientY - rect.top;
        if (!pointer.down) {
          updatePointerDownData(pointer, touches[i].identifier, offsetX, offsetY);
        }
        updatePointerMoveData(pointer, offsetX, offsetY);
        if (pointer.moved) {
          pointer.color = { ...themeColor };
          splat(pointer.texcoordX, pointer.texcoordY, pointer.deltaX * config.SPLAT_FORCE, pointer.deltaY * config.SPLAT_FORCE, pointer.color);
        }
  ...
  }, [getWebGLContext, themeColor]);

  return (
    <canvas
      ref={canvasRef}
      className="fixed inset-0 w-full h-full pointer-events-auto z-[1] hidden lg:block"
      style={{ 
        opacity: 0.6,
        mixBlendMode: "screen",
      }}
