...
export const Navbar = () => {
  ...
  return <motion.nav initial={{
    ...
    duration: 0.5
  }} className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "glass-card border-b border-border/50" : "bg-transparent"}`}>
        ...
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <a href="#home" className="flex items-center gap-3 group">
            ...
            <div className="hidden sm:block">
              <h1 className="font-semibold text-foreground">Anish Guragain</h1>
              <p className="text-xs font-mono text-muted-foreground">Web Developer</p>
            </div>
          </a>

          {/* Desktop Navigation */}
          <div className="hidden lg:flex items-center gap-8">
            {navLinks.map(link => <a key={link.name} href={link.href} className="text-sm font-medium text-muted-foreground hover:text-primary transition-colors relative group">
                {link.name}
      ...
      <AnimatePresence>
        ...
        height: 0
            ...
            delay: index * 0.1
          }} onClick={() => setIsMobileMenuOpen(false)} className="text-lg font-medium text-muted-foreground hover:text-primary transition-colors py-2">
                  {link.name}
                </motion.a>)}
